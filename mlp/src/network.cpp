...
void network::serialize(std::ostream &out) const{
	out << cant_entradas << ' ' << cant_salidas << '\t';
	out << red.size() << '\n';
...
network::network(std::istream &in){
	size_t size;
	in >> cant_entradas >> cant_salidas;
	cin >> size;
	red.reserve(size);
	for(size_t K=0; K<size; ++K)
		red.push_back( capa(in) );

	cache_input.push_back( vector(cant_entradas+1) ); //cache de las salidas de las capas 
	cache_delta.push_back( vector(cant_entradas+1) );

	for(size_t K=0; K<size; ++K){
		cache_input.push_back( vector(red[K].size()+1) );
		cache_delta.push_back( vector(red[K].size()+1) );
	}
}
